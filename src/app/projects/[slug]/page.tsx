    ...
    <main className="inner-page case-page">
      ...
      <section className="case-layout section">
        ...
        <div className="case-content">
          ...
          <section id="results"><p className="kicker">03 / Проверяемый результат</p><h2>Не обещания, а следы работы</h2><div className="evidence-grid">{project.evidence.map((item) => <article key={item.label}><span>{item.label}</span><strong>{item.value}</strong></article>)}</div><ul className="result-list">{project.results.map((result) => <li key={result}><ShieldCheck aria-hidden="true" />{result}</li>)}</ul></section>
